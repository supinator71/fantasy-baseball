...
export function LeagueProvider({ children }) {
   ...
   * force=true:  bypasses cache, forces fresh Haiku analysis (~$0.010/call)
   */
  ...
  const fetchLeagueData = useCallback(async (key) => {
    setLoading(true);
  ...
  const fetchLeagues = useCallback(async () => {
    try {
      ...
      setLeagues(data);
      if (data[0]?.league_key) {
        setSelectedLeague(prev => prev || data[0].league_key);
      }
    } catch (err) {
  ...
  const refreshAnalysis = useCallback(() => {
    if (selectedLeague) runAnalysis(selectedLeague, true); // force=true → bypasses cache and updates live
