...
function streamingValue(pitcher = {}, opposingTeamStats = {}, scoringType = 'Points') {
  ...
  // Opponent offensive quality via xwOBA (League Avg baseline 0.315)
  const oppXwOBA = opposingTeamStats.xwoba ?? opposingTeamStats.xwOBA ?? 0.315;
  if (oppXwOBA < 0.300) score += 15;
  else if (oppXwOBA > 0.330) score -= 15;

  // Normalized Deception and Movement
  const pitcherStats = pitcher.stats || pitcher;
  const totalSwings = pitcherStats.total_swings ?? 1;
  const swordRate = (pitcherStats.swords ?? 0) / totalSwings;
  const movement = pitcherStats.pitch_movement ?? 0;

  if (swordRate > 0.035 && movement > 1.5) score += 20; 
  else if (swordRate < 0.01) score -= 15;

  if (fmt === FORMAT.H2H_POINTS) {
    // Pitcher Defensive Range/Mobility (Bolts)
    // High-Bolt pitchers (like Ohtani) provide value in fielding range and neutralizing fast teams.
    const bolts = pitcherStats.bolts ?? 0;
    if (bolts > 1) score += 10; 
  }

  return { 
    score: Math.min(100, Math.max(0, Math.round(score))), 
    grade: score >= 75 ? 'Elite stream' : score >= 60 ? 'Good stream' : 'Avoid'
  };
}
...
function scoreWaiverTarget(player = {}, myRoster = [], leagueSettings = {}, categoryNeeds = null, pitchingContext = null) {
  ...
  // ── 1. Trend Analysis (Requires minimum 20 PA/IP for significance) ─────
  const sampleSize = isPitcher 
    ? (player.recent_ip ?? player.stats?.recent_ip ?? 0) 
    : (player.recent_pa ?? player.stats?.recent_pa ?? 0);
  
  let trendTriggered = false;
  let recentVal = 0;
  let seasonVal = 0;

  if (sampleSize >= 20) {
    const recentX = isPitcher 
      ? (player.recent_xera ?? player.stats?.recent_xera ?? LEAGUE_AVG.ERA) 
      : (player.recent_xwoba ?? player.stats?.recent_xwoba ?? 0.315);
    const seasonX = isPitcher 
      ? (player.season_xera ?? player.stats?.season_xera ?? LEAGUE_AVG.ERA) 
      : (player.season_xwoba ?? player.stats?.season_xwoba ?? 0.315);
    
    recentVal = recentX;
    seasonVal = seasonX;

    // Check for fundamental improvement vs. baseline
    if (isPitcher ? recentX < seasonX * 0.85 : recentX > seasonX * 1.15) {
      score += 25; 
      trendTriggered = true;
    }
  }

  // ── 2. Underlying Statcast Flags ───────────────────────────────────────
  const statsObj = player.stats || {};
  let statFlags = [];

  if (!isPitcher) {
    const bbe = statsObj.bbe ?? 1;
    const blastRate = (statsObj.blasts ?? 0) / bbe;
    const batSpeed = statsObj.bat_speed ?? 0;
    const bolts = statsObj.bolts ?? 0;
    
    if (blastRate > 0.12) {
      score += 15;   // High-value contact rate
      statFlags.push(`BlastRate ${(blastRate * 100).toFixed(0)}%`);
    }
    if (bolts > 3) {
      score += 15;    // Pure athletic speed floor
      statFlags.push(`Bolts ${bolts}`);
    }
    if (batSpeed > 77) {
      score += 10; // Elite physical tool
      statFlags.push(`BatSpeed ${batSpeed}mph`);
    }
  } else {
    const totalSwings = statsObj.total_swings ?? 1;
    const swordRate = (statsObj.swords ?? 0) / totalSwings;
    const movement = statsObj.pitch_movement ?? 0;

    if (swordRate > 0.03) {
      score += 15;  // Inducing non-competitive swings
      statFlags.push(`SwordRate ${(swordRate * 100).toFixed(1)}%`);
    }
    if (movement > 2.5) {
      score += 10; // Extreme movement
      statFlags.push(`Movement ${movement.toFixed(1)}in`);
    }
  }
  ...
  if (reasoning === '') {
    let statMsg = '';
    if (trendTriggered) {
      statMsg += isPitcher ? `xERA improved (${recentVal.toFixed(2)} vs ${seasonVal.toFixed(2)})` : `xwOBA improved (${recentVal.toFixed(3)} vs ${seasonVal.toFixed(3)})`;
    }
    if (statFlags.length > 0) {
      if (statMsg) statMsg += ', ';
      statMsg += `Statcast: ${statFlags.join(', ')}`;
    }
    if (!statMsg) {
      statMsg = 'no major Statcast indicators';
    }

    reasoning = `Positional need (${pos}: ${countAtPos}/${required}), schedule (${weekGames} games)${catNote}, ${statMsg}`;
  }
...
function detectBreakoutRegression(playerStats = {}, type = 'hitter', scoringType = 'Roto') {
  ...
  // Data Safety & Regression Anchors
  // Support both direct properties and stats subproperty
  const statsObj = playerStats.stats || playerStats;
  const xwOBA = statsObj.xwoba ?? statsObj.xwOBA ?? 0.315;
  const wOBA = statsObj.woba ?? statsObj.wOBA ?? 0.315;
  const batSpeed = statsObj.bat_speed ?? 0;
  const bbe = statsObj.bbe ?? 1; // Avoid division by zero
  const blastRate = (statsObj.blasts ?? 0) / bbe;

  if (type === 'hitter') {
    // Production vs. Expected Delta (0.030 is ~1 StdDev)
    if (xwOBA > wOBA + 0.030) {
      breakoutScore += 15;
      flags.push({ stat: 'xwOBA', verdict: 'Underperforming quality of contact; positive regression likely.' });
    } else if (wOBA > xwOBA + 0.030) {
      breakoutScore -= 15;
      flags.push({ stat: 'xwOBA', verdict: 'Overperforming quality of contact; negative regression likely.' });
    }

    // Power Sustainability via Elite Bat Speed (75+ mph) & normalized Blast Rate
    if (batSpeed > 75) { 
      breakoutScore += 10;
      if (blastRate > 0.15) { // 15% Blast-per-BBE is elite
        breakoutScore += 15;
        flags.push({ stat: 'BlastRate', verdict: 'Elite power/contact efficiency combo identifies high-floor breakout.' });
      }
    } else if (batSpeed > 0 && batSpeed < 68) {
      breakoutScore -= 10;
    }
  } else {
    // Pitcher analysis using LEAGUE_AVG object from Source Context
    const xERA = statsObj.xera ?? statsObj.xERA ?? LEAGUE_AVG.ERA;
    const era = statsObj.era ?? statsObj.ERA ?? LEAGUE_AVG.ERA;
    const totalSwings = statsObj.total_swings ?? 1;
    const swordRate = (statsObj.swords ?? 0) / totalSwings;
    const movement = statsObj.pitch_movement ?? 0;
    ...
    if (xERA < era - 0.50) {
      breakoutScore += 15;
      flags.push({ stat: 'xERA', verdict: `Peripheral dominance suggests ERA will regress toward ${xERA.toFixed(2)}.` });
    } else if (xERA > era + 0.50) {
      breakoutScore -= 15;
      flags.push({ stat: 'xERA', verdict: `Peripheral struggles suggest ERA will regress toward ${xERA.toFixed(2)}.` });
    }

    // Deception via Sword Rate
    if (swordRate > 0.04) { // Swords on 4%+ of swings is elite
      breakoutScore += 15;
      flags.push({ stat: 'SwordRate', verdict: 'High non-competitive swing rate suggests elite deception.' });
    } else if (totalSwings > 10 && swordRate < 0.01) {
      breakoutScore -= 15;
      flags.push({ stat: 'SwordRate', verdict: 'Low non-competitive swing rate suggests poor deception.' });
    }
    
    if (movement > 2.0) { // 2+ inches above league average
      breakoutScore += 10;
      flags.push({ stat: 'Movement', verdict: 'Significant movement profile sustains high K-rates.' });
    }
  }

  const verdict = breakoutScore >= 20 ? 'STRONG BREAKOUT CANDIDATE' : 
                  breakoutScore >= 10 ? 'MILD BREAKOUT CANDIDATE' : 
                  breakoutScore <= -20 ? 'HIGH REGRESSION RISK' : 'SUSTAINABLE PROFILE';
