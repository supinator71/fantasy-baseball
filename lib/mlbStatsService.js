...
export async function getLiveProbablePitchers() {
  ...
  if (cached) return cached;
  try {
    const todayStr = getNYTodayStr();
    const { data } = await axios.get(`${BASE_URL}/schedule`, { params: { sportId: 1, date: todayStr, hydrate: 'probablePitcher' }, timeout: 8000 });
...
export async function getTodayTeamsPlaying() {
  ...
  try {
    ...
    const { data } = await axios.get(`${BASE_URL}/schedule`, {
      params: { sportId: 1, date: todayStr, hydrate: 'team' },
      timeout: 8000,
    });
    const abbrs = new Set();
    const names = new Set();
...
export async function getTodayLiveScores() {
  ...
  try {
    const todayStr = getNYTodayStr();
    const { data } = await axios.get(`${BASE_URL}/schedule`, { params: { sportId: 1, date: todayStr, hydrate: 'decidingPitcher,linescore' }, timeout: 10000 });
    const results = [];
    const games = data.dates?.[0]?.games || [];
    games.forEach(g => {
      results.push({
...
export async function getTwoStartPitchers() {
  ...
  try {
    ...
    const weekday = weekdayMap[wdStr];

    const todayStr = `${y}-${String(m).padStart(2, '0')}-${String(d).padStart(2, '0')}`;

    const nyNoon = new Date(Date.UTC(y, m - 1, d, 12, 0, 0));
    const daysSinceMonday = weekday === 0 ? 6 : weekday - 1;

    const mondayNY = new Date(nyNoon);
    mondayNY.setUTCDate(nyNoon.getUTCDate() - daysSinceMonday);
    const cMonStr = mondayNY.toISOString().split('T')[0];

    const sundayNY = new Date(mondayNY);
    sundayNY.setUTCDate(mondayNY.getUTCDate() + 6);
    const cSunStr = sundayNY.toISOString().split('T')[0];

    const nextMondayNY = new Date(mondayNY);
    nextMondayNY.setUTCDate(mondayNY.getUTCDate() + 7);
    const nMonStr = nextMondayNY.toISOString().split('T')[0];

    const nextSundayNY = new Date(nextMondayNY);
    nextSundayNY.setUTCDate(nextMondayNY.getUTCDate() + 6);
    const nSunStr = nextSundayNY.toISOString().split('T')[0];
