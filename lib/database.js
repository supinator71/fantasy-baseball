...
export const db = {
  ...
  awardCard(yahooGuid, cardId, reason) {
    ...
    // 1. Increment global serial count for this card
    if (!data.global_stats.cards_issued[cardId]) data.global_stats.cards_issued[cardId] = 0;
    data.global_stats.cards_issued[cardId]++;
    const serialNumber = data.global_stats.cards_issued[cardId];

    // 2. Assign to a random Cyber-League team
    const teams = [
      "Brooklyn Biotics", "Tokyo Tachyons", "Neon City Sliders", "Silicon Valley Sentinels",
      "Dallas Tex-Mechs", "Osaka Overclockers", "Kyoto Kaiju", "Roswell Rayguns",
      "Atlanta Aerodynamics", "Miami Motherboards", "San Juan Synthetics", "Havana Hover-Hounds"
    ];
    const team = teams[Math.floor(Math.random() * teams.length)];

    // 3. Save to user's trophy case
    let tc = data.trophy_cases[yahooGuid] || { unlocked_cards: [], last_daily_pack: null };
    const cardNumber = Math.floor(Math.random() * 999) + 1;
    const serialPosition = Math.floor(Math.random() * (data.global_stats.cards_issued[cardId] || 1)) + 1;
    ...
    return { id: cardId, serial: serialNumber, serialPosition, cardNumber, team };
  },
