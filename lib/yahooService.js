...
export async function getPlayers(guid, leagueKey, status = 'A', start = 0, position = null) {
  const queryStatus = status || 'A';
  const posFilter = position ? `;position=${position}` : '';
  const data = await yahooGet(guid, `/league/${leagueKey}/players${posFilter};status=${queryStatus};sort=AR;start=${start};count=25/stats`);
...
export async function searchPlayers(guid, leagueKey, searchStr) {
  const data = await yahooGet(guid, `/league/${leagueKey}/players;search=${encodeURIComponent(searchStr)}/stats`);
  const leagueObj = data.fantasy_content?.league;
  const rawPlayers =
    leagueObj?.[1]?.players ||
    leagueObj?.[0]?.players ||
...
export async function getBatchPlayerStats(guid, leagueKey, playerKeys, type) {
  ...
  // Yahoo caps player_keys at 25 per request. The old code did
  // playerKeys.slice(0, 25) — silently DROPPING every key past 25, which is
  // how a 27-man roster (IL players sort last) lost its last 2 players in the
  // snapshot export ("goinyard bug", found 2026-07-02, fixed 2026-07-03).
  // Chunk into batches of 25 and concatenate instead.
  const result = [];
  for (let i = 0; i < playerKeys.length; i += 25) {
    ...
    result.push(...parsePlayersStats(data.fantasy_content?.league?.[1]?.players));
  }
  return result;
}
