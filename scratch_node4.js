...
const repStr = `  const obviousStarts = enriched.filter(p => !String(p.status).includes('IL') && p.vor >= 60 && ['No', '0'].indexOf(p.is_starting) === -1);
  const obviousSits = enriched.filter(p => String(p.status).includes('IL') || ['No', '0'].indexOf(p.is_starting) !== -1 || (p.vor < 30 && p.streaming?.score < 50));
...
if (content.includes(targetStr)) {
  ...
  console.log('Update Complete Starts/Sits');
} else {
