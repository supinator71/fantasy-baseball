...
export async function GET(request) {
  ...
  const redirectUri = process.env.YAHOO_REDIRECT_URI?.trim() || 'https://localhost:3000/auth/yahoo/callback';
  ...
  try {
    ...
    const { data } = await axios.post('https://api.login.yahoo.com/oauth2/get_token', 
      new URLSearchParams({
        client_id: clientId,
    ...
    if (!guid) {
      ...
      try {
        const profileResponse = await axios.get('https://fantasysports.yahooapis.com/fantasy/v2/users;use_login=1?format=json', {
    ...
    if (!guid) {
      throw new Error('Failed to retrieve Yahoo GUID');
    }
